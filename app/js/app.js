...
ScrollTrigger.matchMedia({
  all: function () {
    ...
    // Main secction bg
    const sectionBg = gsap.utils.toArray(".pagination__item");
    const rule1 = CSSRulePlugin.getRule(".main__section_bg::before");
    sectionBg.forEach((panel, i) => {
      ScrollTrigger.create({
        trigger: panel,
        start: "top center",
        ...
        onEnter: () => {
          ...
          sectionBg[i].classList.add("active");
        },
        onEnterBack: () => {
          sectionBg.forEach((e) => {
            e.classList.remove("active");
...
// Swiper interior
document
  .querySelectorAll(".main__floorplans .swiper")
